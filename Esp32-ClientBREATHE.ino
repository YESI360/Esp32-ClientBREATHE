...
void setup()
{
  ...
  if (connected)
  {
    ...
    String WS_msg = String("Hello to Server from ") + sensorValue;//////
    client.send(WS_msg);
  }
  else
  {
...
void loop()
{
  String WS_msg = String(" yesi ") + sensorValue;//////
  //WS_msg = sensorValue;//////
  client.send(WS_msg);
  LecturaAnalog ();
...
void LecturaAnalog ()
{
  ...
  //  outputValue = map(sensorValue, 0, 1023, 1, 20);//mapeo
  //
